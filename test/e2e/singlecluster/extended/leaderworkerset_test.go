...
var _ = ginkgo.Describe("LeaderWorkerSet integration", ginkgo.Label("area:singlecluster", "feature:leaderworkerset"), func() {
	...
	ginkgo.When("LeaderWorkerSet created with Restart Policy", func() {
		ginkgo.It("should recreate pods when policy is set to RecreateGroupOnPodRestart", func() {
			...
			// LWS recreates the group either when a pod is deleted or when a container
			// restarts. Restart the container rather than delete the pod: the deletion path
			// reads the transient DeletionTimestamp, which the controller can miss once the
			// StatefulSet recreates the pod under the same name, while RestartCount is durable
			// status that cannot be missed.
			// TODO: revisit once https://github.com/kubernetes-sigs/lws/issues/998 is fixed.
